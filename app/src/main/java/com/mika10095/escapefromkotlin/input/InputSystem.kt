...
class InputSystem(context: Context, var settingsManager: SettingsManager, width: Int, height: Int) {
    ...
    val backButton =
        RectF(0f, height - height.toFloat() / 2f, width.toFloat() / 3f, height.toFloat())
    val shootButton = RectF(width / 2f, 0f, width.toFloat(), height.toFloat() * (3f / 4f))
    val turnLeftButton = RectF(
        width / 2f,
        height.toFloat() * (3f / 4f),
        width.toFloat() * (3f / 4f),
        height.toFloat()
    )
    val turnRightButton = RectF(
        width.toFloat() * (3f / 4f),
        height.toFloat() * (3f / 4f),
        width.toFloat(),
        height.toFloat()
    ...
    val mapButton = RectF(
        ...
        0f,
        width.toFloat() * (7f / 8f),
        width.toFloat() * (7f / 8f) - width.toFloat() * (3f / 4f)
    )
    val menuButton = RectF(
        width.toFloat() * (7f / 8f),
        0f,
        width.toFloat(),
        width.toFloat() * (7f / 8f) - width.toFloat() * (3f / 4f)
    ...
    val pistolButton =
        RectF(0f, height.toFloat() / 4, height.toFloat() / 4f, 2 * height.toFloat() / 4)
    val shotgunButton =
        RectF(0f, 2 * height.toFloat() / 4, height.toFloat() / 4f, 3 * height.toFloat() / 4)
    val launcherButton =
        RectF(0f, 3 * height.toFloat() / 4, height.toFloat() / 4f, height.toFloat())
    ...
    val shotgunButtonSprite =
        BitmapFactory.decodeResource(context.resources, R.drawable.shotgun_icon)
    val launcherButtonSprite =
        BitmapFactory.decodeResource(context.resources, R.drawable.panzerfaust_icon)
    val healthIconHolder =
        RectF(0f, 3f / 4f * height.toFloat(), height.toFloat() / 4f, height.toFloat())
    val armorIconHolder = RectF(
        height.toFloat() / 4f,
        3f / 4f * height.toFloat(),
        2 * height.toFloat() / 4f,
        height.toFloat()
    )
    val ammoIconHolder = RectF(
        2 * height.toFloat() / 4f,
        3f / 4f * height.toFloat(),
        3 * height.toFloat() / 4f,
        height.toFloat()
